...
-- ═══════════════════════════════════════
-- SERVICE ROLE: Full access (used by Cloudflare Worker)
-- The Worker uses SUPABASE_SERVICE_KEY which bypasses RLS entirely.
-- These policies are for the anon key (used by browser clients).
-- ═══════════════════════════════════════

-- ANON KEY POLICIES (browser-side — minimal access)
-- Only allow token verification and platform config reads from browser
CREATE POLICY "Anon can verify tokens" ON access_tokens FOR SELECT USING (true);
CREATE POLICY "Anon can read platform config" ON platform_config FOR SELECT USING (true);

-- Block anon from all other tables (service role bypasses RLS)
...
-- All other tables: no anon access (service role in Worker handles everything)
